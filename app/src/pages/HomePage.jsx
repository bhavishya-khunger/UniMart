...
import ShopPrintOrders from "../components/HomePage/ShopPrintOrders.jsx";
...
function App() {
  ...
  const getShopOrders = async () => {
    if (user?.shopType === "print") {
      try {
        const res = await axios.get(`${import.meta.env.VITE_SHOP_BASE_URL}/get-print-orders/${user?._id}`);
          ...
          <div className="max-w-7xl mx-auto p-4">
            {shopOrders?.length ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {shopType === "print" ?
                  <ShopPrintOrders sortedOrders={shopOrders} user={user} />
                  :
